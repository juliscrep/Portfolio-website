...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        if (navbarOpen) {
            document.addEventListener('click', closeNavbarOnLinkClick);
        }
        return () => {
            document.removeEventListener('click', closeNavbarOnLinkClick);
        };
    }, [navbarOpen]);
    ...
    return (
        <nav className='fixed mx-auto border border-[#33353F] top-0 left-0 right-0 z-10 bg-[#121212] bg-opacity-100'>
            <div className='flex container lg:py-6 flex-wrap items-center justify-between mx-auto px-4 py-2'>
                ...
                <div className={`menu ${navbarOpen ? 'hidden' : 'hidden'} md:block md:w-auto`} id="navbar">
                    <ul className='flex p-4 mr-4 md:p-0 md:flex-row md:space-x-8 mt-0'>
                        {
